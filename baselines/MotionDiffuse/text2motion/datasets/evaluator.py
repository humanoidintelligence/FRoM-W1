...
class EvaluationDataset(Dataset):
    ...
    def __init__(self, opt, trainer, dataset, w_vectorizer, mm_num_samples, mm_num_repeats):
        ...
        dataloader = DataLoader(dataset, batch_size=1, num_workers=1, shuffle=False)  # NOTE(pli): shuffle=True before.
        ...
        min_mov_length = 15 if opt.dataset_name == 't2mx' else 6
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader), desc="loading dataset"):
        ...
        # print ("* " * 30)
        # print (f"==> sample num: {len(all_m_lens)}")
        # print ("* " * 30)
        ...
        mm_generated_motions = []
        with torch.no_grad():
            print ("loading pred motions ...")
            for i, data_dummy in tqdm(enumerate(dataloader), desc="predicting dataset"):
                data = all_data[i]
                word_emb, pos_ohot, caption, cap_lens, motions, m_lens, tokens = data
                tokens = tokens[0].split('_')
                mm_num_now = len(mm_generated_motions)
...
class Text2MotionDatasetV2(Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 28 # 30 before.
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        min_motion_len = 60 if self.opt.dataset_name =='t2mx' else 24
        ...
        new_name_list = []
        length_list = []
        print ("Loading Text2MotionDatasetV2 ...")
...
def get_dataset_motion_loader(opt_path, batch_size, device):
    ...
    if 't2m' in opt.dataset_name or opt.dataset_name == 'kit':
...
def build_models(opt):
    ...
    checkpoint = torch.load(pjoin('checkpoints', opt.dataset_name, 'text_mot_match', 'model', 'finest.tar'),
