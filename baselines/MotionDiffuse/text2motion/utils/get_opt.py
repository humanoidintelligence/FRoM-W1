...
def get_opt(opt_path, device):
    ...
    opt_dict['which_epoch'] = 'latest' # 'ckpt_e290'
    ...
    #     opt_dict['diffusion_steps'] = 50 # 1000 for train, 50 for inference.
    if 'no_clip' not in opt_dict:
        opt_dict['no_clip'] = False
    if 'no_eff' not in opt_dict:
        opt_dict['no_eff'] = False
    
    opt_dict['diffusion_steps'] = 1000 # 1000 for train, 50 for inference. 10 for dev
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.data_root = './data/HumanML3D-X'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 52
        opt.dim_pose = 623
