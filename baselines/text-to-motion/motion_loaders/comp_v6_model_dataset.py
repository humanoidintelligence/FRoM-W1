...
def build_models(opt):
    ...
    # Dev_X_Dist_Length_Est_BiGRU_BS256
    checkpoints = torch.load(pjoin(opt.checkpoints_dir, opt.dataset_name, opt.length_est_name, 'model', 'latest.tar'), map_location=opt.device)
...
class CompV6GeneratedDataset(Dataset):
    ...
    def __init__(self, opt, dataset, w_vectorizer, mm_num_samples, mm_num_repeats):
        ...
        # min_mov_length = 10 if 't2m' in opt.dataset_name else 6
        min_mov_length = 15 if 't2m' in opt.dataset_name else 6
        # print(mm_idxs)
