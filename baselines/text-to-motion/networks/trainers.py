...
from data.dataset import collate_fn, custom_collate_fn
import codecs as cs
    ...
    def train(self, train_dataloader, val_dataloader, 
        ...
        while epoch < self.opt.max_epoch:
            ...
            if epoch % self.opt.eval_every_e == 0:
                ...
                plot_eval(data,  mean, std, fps, radius, save_dir)


class DecompTrainerV3Dist(object):
    def __init__(self, args, movement_enc, movement_dec):
        self.opt = args
        self.movement_enc = movement_enc
        self.movement_dec = movement_dec
        self.device = args.device
        self.rank = args.gpu_id  # 添加rank信息
    ...
    def save(self, file_name, ep, total_it):
        ...
        # 使用module属性获取原始模型状态
        state = {
            'movement_enc': self.movement_enc.module.state_dict(),
            'movement_dec': self.movement_dec.module.state_dict(),
    ...
    def resume(self, model_dir):
        ...
        # 处理DDP包装的模型
        if hasattr(self.movement_enc, 'module'):
            self.movement_enc.module.load_state_dict(checkpoint['movement_enc'])
            self.movement_dec.module.load_state_dict(checkpoint['movement_dec'])
        else:
            self.movement_enc.load_state_dict(checkpoint['movement_enc'])
            self.movement_dec.load_state_dict(checkpoint['movement_dec'])
    ...
    def train(self, train_dataloader, val_dataloader, 
              kinematic_chain, mean, std, fps, radius, plot_eval):
        # 模型已经在主函数中移动到设备并用DDP包装
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                it += 1
                if it % self.opt.log_every == 0 and self.rank == 0:  # 只在主进程上记录日志
                    mean_loss = OrderedDict({'val_loss': val_loss})
                ...
                if it % self.opt.save_latest == 0 and self.rank == 0:  # 只在主进程上保存
                    self.save(pjoin(self.opt.model_dir, 'latest.tar'), epoch, it)
            ...
            # 只在主进程上保存周期模型
            if epoch % self.opt.save_every_e == 0 and self.rank == 0:
                self.save(pjoin(self.opt.model_dir, 'E%04d.tar' % (epoch)), epoch, total_it=it)
            ...
            if dist.is_available() and dist.is_initialized():
                ...
                val_rec_loss_tensor = torch.tensor(val_rec_loss).to(self.device)
                val_sparcity_loss_tensor = torch.tensor(val_sparcity_loss).to(self.device)
                val_smooth_loss_tensor = torch.tensor(val_smooth_loss).to(self.device)
                
                dist.all_reduce(val_loss_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(val_rec_loss_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(val_sparcity_loss_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(val_smooth_loss_tensor, op=dist.ReduceOp.SUM)
                
                world_size = dist.get_world_size()
                val_loss = val_loss_tensor.item() / (len(val_dataloader) * world_size + 1)
                val_rec_loss = val_rec_loss_tensor.item() / (len(val_dataloader) * world_size + 1)
                val_sparcity_loss = val_sparcity_loss_tensor.item() / (len(val_dataloader) * world_size + 1)
                val_smooth_loss = val_smooth_loss_tensor.item() / (len(val_dataloader) * world_size + 1)
            else:
                val_loss = val_loss / (len(val_dataloader) + 1)
                val_rec_loss = val_rec_loss / (len(val_dataloader) + 1)
                val_sparcity_loss = val_sparcity_loss / (len(val_dataloader) + 1)
            ...
            if self.rank == 0:
                print('Validation Loss: %.5f Reconstruction Loss: %.5f '
                      'Sparsity Loss: %.5f Smooth Loss: %.5f' % (val_loss, val_rec_loss, val_sparcity_loss, val_smooth_loss))

            # 只在主进程上执行评估和可视化
            if epoch % self.opt.eval_every_e == 0 and self.rank == 0:
                data = torch.cat([self.recon_motions[:4], self.motions[:4]], dim=0).detach().cpu().numpy()
                save_dir = pjoin(self.opt.eval_dir, 'E%04d' % (epoch))
                os.makedirs(save_dir, exist_ok=True)
                plot_eval(data, kinematic_chain, mean, std, fps, radius, save_dir)
...
class CompTrainerV6(object):
    ...
    def train(self, train_dataset, val_dataset, plot_eval):
        ...
        epoch = 0
        it = 0
        if 't2m' in self.opt.dataset_name:
            schedule_len = 15 # 10 before
...
class CompTrainerV6Dist(object):
    ...
    def forward(self, batch_data, tf_ratio, mov_len, eval_mode=False):
        ...
        if self.opt.text_enc_mod == 'bigru':
            hidden_pos = self.seq_post.module.get_init_hidden(hidden)
            hidden_pri = self.seq_pri.module.get_init_hidden(hidden)
            hidden_dec = self.seq_dec.module.get_init_hidden(hidden)
        elif self.opt.text_enc_mod == 'transformer':
            hidden_pos = self.seq_post.module.get_init_hidden(hidden.detach())
            hidden_pri = self.seq_pri.module.get_init_hidden(hidden.detach())
            hidden_dec = self.seq_dec.module.get_init_hidden(hidden)
    ...
    def save(self, file_name, ep, total_it, sub_ep, sl_len):
        ...
        # 使用module属性获取原始模型状态
        state = {
            'text_enc': self.text_enc.module.state_dict(),
            'seq_post': self.seq_post.module.state_dict(),
            'att_layer': self.att_layer.module.state_dict(),
            'seq_dec': self.seq_dec.module.state_dict(),
            'seq_pri': self.seq_pri.module.state_dict(),
            'mov_enc': self.mov_enc.module.state_dict(),
            'mov_dec': self.mov_dec.module.state_dict(),
    ...
    def load(self, model_dir):
        ...
        # 处理DDP包装的模型
        if hasattr(self.text_enc, 'module'):
            self.text_enc.module.load_state_dict(checkpoint['text_enc'])
            self.mov_dec.module.load_state_dict(checkpoint['mov_dec'])
            self.seq_pri.module.load_state_dict(checkpoint['seq_pri'])
            self.att_layer.module.load_state_dict(checkpoint['att_layer'])
            self.seq_dec.module.load_state_dict(checkpoint['seq_dec'])
            self.mov_enc.module.load_state_dict(checkpoint['mov_enc'])
            if self.opt.is_train:
                self.seq_post.module.load_state_dict(checkpoint['seq_post'])
        else:
            self.text_enc.load_state_dict(checkpoint['text_enc'])
            self.mov_dec.load_state_dict(checkpoint['mov_dec'])
            self.seq_pri.load_state_dict(checkpoint['seq_pri'])
    ...
    def train(self, train_dataset, val_dataset, per_gpu_batch_size, mean, std, 
                  kinematic_chain, fps, radius, joints_num, plot_eval):
        self.to(self.device)
        ...
        while invalid:
            ...
            # 使用分布式采样器
            world_size = dist.get_world_size()
            train_sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=self.rank, shuffle=True) if dist.is_initialized() else None
            val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=self.rank, shuffle=False) if dist.is_initialized() else None
            ...
            for sub_epoch in range(sub_ep, self.opt.max_sub_epoch):
                ...
                if train_sampler is not None:
                    train_sampler.set_epoch(epoch * self.opt.max_sub_epoch + sub_epoch)
                ...
                for i in range(len(train_loader)):
                    ...
                    #     continue
                    # batch_data = safe_next_data_loader(train_loader, train_loader_iter)
                    
                    batch_data = next(train_loader_iter)
                    # continue
                    # if i % 10 == 0:
                    #     print (f"temp: {schedule_len} | {self.opt.max_sub_epoch} | {sub_epoch} | {len(train_loader)} | {i}, good luck.") 
                    # continue
                ...
                epoch += 1
                if epoch % self.opt.save_every_e == 0 and self.rank == 0:  # 只在主进程上保存周期模型
                    self.save(pjoin(self.opt.model_dir, 'E%03d_SE%02d_SL%02d.tar'%(epoch, sub_epoch, schedule_len)),
                              epoch, total_it=it, sub_ep=sub_epoch, sl_len=schedule_len)
                ...
                if dist.is_available() and dist.is_initialized():
                    # 将所有进程的损失汇总
                    loss_mot_rec_tensor = torch.tensor(loss_mot_rec).to(self.device)
                    loss_mov_rec_tensor = torch.tensor(loss_mov_rec).to(self.device)
                    loss_kld_tensor = torch.tensor(loss_kld).to(self.device)
                    val_loss_tensor = torch.tensor(val_loss).to(self.device)
                    
                    dist.all_reduce(loss_mot_rec_tensor, op=dist.ReduceOp.SUM)
                    dist.all_reduce(loss_mov_rec_tensor, op=dist.ReduceOp.SUM)
                    dist.all_reduce(loss_kld_tensor, op=dist.ReduceOp.SUM)
                    dist.all_reduce(val_loss_tensor, op=dist.ReduceOp.SUM)
                    
                    world_size = dist.get_world_size()
                    loss_mot_rec = loss_mot_rec_tensor.item() / (len(val_loader) * world_size + 1)
                    loss_mov_rec = loss_mov_rec_tensor.item() / (len(val_loader) * world_size + 1)
                    loss_kld = loss_kld_tensor.item() / (len(val_loader) * world_size + 1)
                    val_loss = val_loss_tensor.item() / (len(val_loader) * world_size + 1)
                else:
                    loss_mot_rec /= len(val_loader) + 1
                    loss_mov_rec /= len(val_loader) + 1
                    loss_kld /= len(val_loader) + 1
                ...
                if epoch % self.opt.eval_every_e == 0 and self.rank == 0:
                    ...
                    save_dir = pjoin(self.opt.eval_dir, 'E%03d_SE%02d_SL%02d'%(epoch, sub_epoch, schedule_len))
                    os.makedirs(save_dir, exist_ok=True)
                    plot_eval(data, save_dir, captions, None, mean, std, 
                  kinematic_chain, fps, radius, joints_num)
...
class LengthEstTrainerDist(object):
    ...
    def resume(self, model_dir):
        ...
        # 处理DDP包装的模型
        if hasattr(self.estimator, 'module'):
            self.estimator.module.load_state_dict(checkpoint['estimator'])
        else:
            self.estimator.load_state_dict(checkpoint['estimator'])

        self.opt_estimator.load_state_dict(checkpoint['opt_estimator'])
    ...
    def save(self, model_dir, epoch, niter):
        ...
        # 使用module属性获取原始模型状态
        state = {
            'estimator': self.estimator.module.state_dict(),
            'opt_estimator': self.opt_estimator.state_dict(),
            'epoch': epoch,
            'niter': niter,
        }
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                # # print (pred_dis[0])
                # print (f"gt_labels: {gt_labels.max()}  {gt_labels.min()}")
                # print (f"pred_dis: {pred_dis.max()}  {pred_dis.min()}")
                # # exit(0)
...
class TextMotionMatchTrainerDist(object):
    ...
    def resume(self, model_dir):
        ...
        if hasattr(self.text_encoder, 'module'):
            self.text_encoder.module.load_state_dict(checkpoints['text_encoder'])
            self.motion_encoder.module.load_state_dict(checkpoints['motion_encoder'])
            self.movement_encoder.module.load_state_dict(checkpoints['movement_encoder'])
        else:
            self.text_encoder.load_state_dict(checkpoints['text_encoder'])
            self.motion_encoder.load_state_dict(checkpoints['motion_encoder'])
            self.movement_encoder.load_state_dict(checkpoints['movement_encoder'])
    ...
    def save(self, model_dir, epoch, niter):
        ...
        state = {
            ...
            'motion_encoder': self.motion_encoder.module.state_dict(),
            'movement_encoder': self.movement_encoder.module.state_dict(),
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        while epoch < self.opt.max_epoch:
            ...
            if dist.is_available() and dist.is_initialized():
                # 将所有进程的损失汇总
                val_loss_tensor = torch.tensor(val_loss).to(self.device)
                loss_pos_pair_tensor = torch.tensor(loss_pos_pair).to(self.device)
                loss_neg_pair_tensor = torch.tensor(loss_neg_pair).to(self.device)
                
                dist.all_reduce(val_loss_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(loss_pos_pair_tensor, op=dist.ReduceOp.SUM)
                dist.all_reduce(loss_neg_pair_tensor, op=dist.ReduceOp.SUM)
                
                world_size = dist.get_world_size()
                val_loss = val_loss_tensor.item() / (len(val_dataloader) * world_size + 1)
                loss_pos_pair = loss_pos_pair_tensor.item() / (len(val_dataloader) * world_size + 1)
                loss_neg_pair = loss_neg_pair_tensor.item() / (len(val_dataloader) * world_size + 1)
            else:
                val_loss = val_loss / (len(val_dataloader) + 1)
                loss_pos_pair = loss_pos_pair / (len(val_dataloader) + 1)
                loss_neg_pair = loss_neg_pair / (len(val_dataloader) + 1)
            ...
            # 只在主进程上保存最佳模型
            if self.rank == 0 and val_loss < min_val_loss:
                self.save(pjoin(self.opt.model_dir, 'finest.tar'), epoch, it)
                min_val_loss = val_loss
