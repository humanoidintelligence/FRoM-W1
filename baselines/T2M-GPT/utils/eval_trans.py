...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 
    ...
    if nb_iter % 10000 == 0:
        torch.save({'trans' : trans.state_dict()}, os.path.join(out_dir, f'net_iter_{nb_iter}.pth'))
...
@torch.no_grad()        
def evaluation_transformer_test(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, 
                                best_top2, best_top3, best_matching, best_multi, clip_model, eval_wrapper, 
                                draw = True, save = True, savegif=False, savenpy=False, is_multimodality=False) : 
    ...
    for batch in tqdm(val_loader, desc="pred val_loader"):
        word_embeddings, pos_one_hots, clip_text, sent_len, pose, m_length, token, name = batch
        bs, seq = pose.shape[:2]
        num_joints = 21 if pose.shape[-1] == 251 else 22
        if pose.shape[-1] == 623:
    ...
    msg = f"--> \t Eva. Iter {nb_iter} :, gt-fid. {gt_fid:.4f}, FID. {fid:.4f}, Diversity Real. {diversity_real:.4f}, Diversity. {diversity:.4f}, R_precision_real. \
        {R_precision_real}, R_precision. {R_precision}, matching_score_real. {matching_score_real}, matching_score_pred. {matching_score_pred}"
    if is_multimodality:
    ...
    trans.train()
    if not is_multimodality:
        return gt_fid, fid, best_iter, diversity, R_precision[0], R_precision[1], R_precision[2], matching_score_pred, multimodality, writer, logger
    else:
        return gt_fid, fid, best_iter, diversity, R_precision, R_precision, R_precision, matching_score_pred, multimodality, writer, logger
