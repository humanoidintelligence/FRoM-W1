...
from .HumanML3DX import HumanML3DXDataModule
from .utils import *
...
def get_mean_std(phase, cfg, dataset_name):
    ...
    assert name in ["t2m", "kit", 'humanml3d_x']
    ...
    if phase in ["val"]:
        if name == 't2m':
            ...
            data_root = pjoin(cfg.model.t2m_path, name, "Comp_v6_KLD005",
                              "meta")
        elif name == 'humanml3d_x':
            data_root = pjoin(cfg.model.t2m_path, name, "Dev_X_30FPS_Dist_Comp_v6_KLD01_BS1024",
                              "meta")
        else:
...
def get_WordVectorizer(cfg, phase, dataset_name):
    if phase not in ["text_only"]:
        if dataset_name.lower() in ["humanml3d", "kit", 'humanml3d_x']:
...
def get_collate_fn(name, phase="train"):
    if name.lower() in ["humanml3d", "kit", 'humanml3d_x']:
        return mld_collate
...
dataset_module_map = {
    ...
    'humanml3d_x': HumanML3DXDataModule
}
motion_subdir = {"humanml3d": "new_joint_vecs", "kit": "new_joint_vecs", 'humanml3d_x': 'new_joint_vecs'}
