...
from phc.utils.motion_lib_robot import MotionLibRobot
...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.trajectories_dim = cfg.extra.dof_num * 3 + 6
        self.trajectories_with_linvel_dim = self.trajectories_dim + 3
        self.gravity_link_idx = cfg.extra.gravity_link_idx
        
        self.upper_link_idx = cfg.extra.upper_link_idx
        self.lower_link_idx = cfg.extra.lower_link_idx
        self.upper_joint_idx = cfg.extra.upper_joint_idx
        self.lower_joint_idx = cfg.extra.lower_joint_idx
        self.vr3_keypoint_idx = cfg.extra.vr3_keypoints_idx
        ...
        self.init_done = True
        self.trajectories = torch.zeros(self.num_envs, self.trajectories_dim * 100).to(self.device) 
        self.trajectories_with_linvel = torch.zeros(self.num_envs, self.trajectories_with_linvel_dim * 100).to(self.device) 
        ...
        if self.cfg.motion.teleop:
            self.extend_body_parent_ids = cfg.extra.extend_parent_ids
            self._track_bodies_id = [self._body_list.index(body_name) for body_name in self.cfg.motion.teleop_selected_keypoints_names]
            self._track_bodies_extend_id = self._track_bodies_id + list(range(len(self._body_list), len(self._body_list) + len(self.extend_body_parent_ids)))
            self.extend_body_pos = torch.tensor(cfg.extra.extend_local_translation).repeat(self.num_envs, 1, 1).to(self.device)
    ...
    def step(self, actions):
        ...
        # if self.cfg.train.distill :
            if  "expert_policy" in self.__dict__:
                ...
                _track_bodies_extend_id_new = [self._body_list.index(body_name) for body_name in self.cfg.motion.teleop_selected_keypoints_names] + list(range(len(self._body_list), len(self._body_list)+len(self.cfg.extra.extend_parent_ids)))
        ...
        self.render()
        actions = actions * 0.8 + self.last_actions * 0.2  
        ...
        if self.cfg.train_velocity_estimation:
            ...
            if self.common_step_counter % 5000 == 0:
                ...
                load_path = os.path.join(
                    LEGGED_GYM_ROOT_DIR, model_dir, "velocity_estimator_" + str(self.common_step_counter) + ".pt"
                )
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(self._rigid_body_rot[:, self.gravity_link_idx, :], self._rigid_body_ang_vel[:, self.gravity_link_idx, :])

        self.projected_gravity[:] = quat_rotate_inverse(self._rigid_body_rot[:, self.gravity_link_idx, :], self.gravity_vec)
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.terminate_by_ref_motion_distance:
                ...
                if self.cfg.asset.local_upper_reward:
                    ...
                    ref_body_pos[:, self.upper_link_idx] -= diff
    ...
    def compute_reward(self):
        ...
        """
        self.rew_buf[:] = 0.
        self.dict = {self.reward_names[idx]: self.episode_sums[self.reward_names[idx]].mean() for idx in range(len(self.reward_names))}
    ...
    def compute_self_and_task_obs(self, ):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.asset.local_upper_reward:
                ...
                ref_body_pos_extend[:, self.upper_link_idx] -= diff
            
            self.marker_coords[:] = ref_body_pos_extend.reshape(B, -1, 3)
            
        if self.cfg.motion.teleop_obs_version == 'v-teleop-extend-vr-max-nolinvel':
            body_pos = self._rigid_body_pos
            body_rot = self._rigid_body_rot
        ...
        if self.cfg.env.num_privileged_obs is not None:
            ...
            self.privileged_info = torch.cat([
                ...
                self._kp_scale, #dof32
                self._kd_scale, #dof32
                self._rfi_lim_scale, #dof32
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_base_com:
            torso_index = self._body_list.index(self.cfg.extra.torso_link)
    ...
    def _compute_torques(self, actions):
        ...
        if control_type=="P":
            p_val = self._kp_scale * self.p_gains*(actions_scaled + self.default_dof_pos - self.dof_pos)
            d_val = -self._kd_scale * self.d_gains*self.dof_vel
            d_val_clipped = d_val
            torques = p_val + d_val_clipped
        elif control_type=="V":
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.motion.teleop_obs_version == 'v-teleop-extend-max-full':
            ...
            max_num_bodies = len(self.cfg.motion.teleop_selected_keypoints_names) + len(self.cfg.extra.extend_parent_ids)
            curr_obs_len = 0
            # body_pos
            ...
            # ref dof pos
            if self.cfg.motion.future_tracks:
                noise_vec[2*self.num_dof + 9 : 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + len(self.cfg.extra.extend_parent_ids)) *3 * 3 * self.cfg.motion.num_traj_samples ] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos 
            else:
                noise_vec[2*self.num_dof + 9 : 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + len(self.cfg.extra.extend_parent_ids)) * 3 * 3] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos    
    ...
    def _init_buffers(self):
        ...
        if self.cfg.motion.teleop:
            ...
            self._load_motion()
            self.marker_coords = torch.zeros(self.num_envs, (self.num_dofs + 1 + len(self.cfg.extra.extend_parent_ids)) * self.cfg.motion.num_traj_samples, 3, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _create_envs(self):
        ...
        for i in range(self.num_envs):
            ...
            if self.cfg.asset.set_dof_properties:
                ...
                dof_props['armature'] = self.cfg.asset.default_dof_prop_armature
            self.gym.set_actor_dof_properties(env_handle, actor_handle, dof_props)
            body_props = self.gym.get_actor_rigid_body_properties(env_handle, actor_handle)
            
            self.mass = torch.tensor([body_prop.mass for body_prop in body_props]).to(self.device)
    ...
    def _reward_lower_action_rate(self):
        # Penalize changes in actions
        return torch.sum( torch.square(self.last_actions[:, self.lower_joint_idx] - self.actions[:, self.lower_joint_idx]), dim=1)

    def _reward_upper_action_rate(self):
        # Penalize changes in actions
        return torch.sum(torch.square(self.last_actions[:, self.upper_joint_idx] - self.actions[:, self.upper_joint_idx]), dim=1)
    ...
    def _reward_teleop_body_position_extend(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_lower = diff_global_body_pos[:, self.lower_link_idx]
        diff_global_body_pos_upper = diff_global_body_pos[:, self.upper_link_idx]
    ...
    def _reward_teleop_body_position_vr_3keypoints(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_vr_3keypoints = diff_global_body_pos[:, self.vr3_keypoint_idx]
    ...
    def _reward_feet_contact(self):
        ...
        motion_res = self._get_state_from_motionlib_cache_trimesh(self.motion_ids, motion_times, offset=offset)
        ref_body_vel_extend = motion_res['body_vel']

        ref_feet_contact = (ref_body_vel_extend[:, self.feet_indices, :2].norm(dim=-1) < 0.1).float()
        real_feet_contact = (self.contact_forces[:, self.feet_indices, :].norm(dim=-1) > 1).float()

        contact_diff = (ref_feet_contact - real_feet_contact).abs()

        return -contact_diff.mean()
