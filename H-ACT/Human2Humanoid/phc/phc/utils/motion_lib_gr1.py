...
from phc.utils.torch_gr1_humanoid_batch import Humanoid_Batch
...
class MotionLibGR1(MotionLibBase):

    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix, masterfoot_conifg=None, min_length=-1, im_eval=False, multi_thread=True, extend_hand = False, extend_head = False, mjcf_file="resources/robots/gr1/gr1t1.xml"):
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            raise NotImplementedError("Fix height is not implemented for GR1")
