...
class MotionLibRobot(MotionLibBase):
    ...
    def __init__(self, motion_file, device, fix_height=FixHeightMode.no_fix, masterfoot_conifg=None, min_length=-1, im_eval=False, multi_thread=True, robotConfig=None):
        ...
        if robotConfig:
            self.mesh_parsers = Humanoid_Batch(robotConfig)
        else:
            raise ValueError("robotConfig is required!")
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            raise NotImplementedError("Fix height is not implemented for Robot")
