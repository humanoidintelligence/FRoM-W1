...
class MANO(nn.Module):
    ...
    def joint_meshes(self,output, radius=.002, vc=colors['green']):
        ...
        # 3: index, 6: middle, 9: pinky, 12: ring, 15: thumb
        joint_mesh = Mesh(vertices=joints[0, [16, 17, 18, 19, 20]], radius=0.002, vc=colors['red'])
        meshes.append(joint_mesh)
        pos = np.diag([1,1,1])[0] * 0.1
        joint_mesh = Mesh(vertices=pos, radius=radius, vc=colors['blue'])
        meshes.append(joint_mesh)
        return  meshes
